...
class F5BigipLtmConnector(BaseConnector):
    ...
    def _quote_path_component(value):
        ...
        return quote_path_component(value)

    def _quote_path_components(self, action_result, **components):
        try:
            return {name: self._quote_path_component(value) for name, value in components.items()}
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            if resp_json and (resp_json.get("code") or resp_json.get("message")):
                ...
                message = "Error occurred while making the request. Status Code: {}. Response Code: {}. Message from server: {}".format(
            ...
            message = (
                "Unknown error occurred while processing the output response from the server. "
                f"Status Code: {r.status_code}. Data from server: {error_message}"
            )
    ...
    def _make_rest_call(self, endpoint, action_result, method="get", data=None, **kwargs):
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR,
                    f"Error occurred while creating the REST URL for the API call. Error Code: {error_code}. Error Message: {error_message}",
                ),
                None,
            )
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR,
                    f"Error occurred while making the REST call to the F5 server. Error Code: {error_code}. Error Message: {error_message}",
                ),
                None,
            )
    ...
    def _handle_remove_node(self, param):
        ...
        # make rest call
        paths = self._quote_path_components(action_result, pool=pool_name, node=node_name, port=port)
        if paths is None:
            return action_result.get_status()
        ret_val, response = self._make_rest_call(
            f"/mgmt/tm/ltm/pool/{paths['pool']}/members/{paths['node']}:{paths['port']}", action_result, method="delete"
        )
    ...
    def _handle_add_node(self, param):
        ...
        port = param["port"]
        partition_name = (
            self._handle_py_ver_compat_for_input_str(self._python_version, param["partition_name"]).replace("\\", "\\\\").replace('"', '\\"')
        )
        ...
        # make rest call
        paths = self._quote_path_components(action_result, pool=pool_name)
        if paths is None:
            return action_result.get_status()
        ret_val, response = self._make_rest_call(f"/mgmt/tm/ltm/pool/{paths['pool']}/members", action_result, method="post", data=json_str)
    ...
    def _handle_create_node(self, param):
        ...
        json_str = f'{{"name": "{node}", "partition": "{partition}", "address": "{address}"}}'
    ...
    def _handle_delete_node(self, param):
        ...
        node_name = self._handle_py_ver_compat_for_input_str(self._python_version, param["node_name"])
        # make rest call
        paths = self._quote_path_components(action_result, node=node_name)
        if paths is None:
            return action_result.get_status()
        ret_val, response = self._make_rest_call(f"/mgmt/tm/ltm/node/{paths['node']}", action_result, method="delete")
    ...
    def _handle_disable_node(self, param):
        ...
        if paths is None:
            return action_result.get_status()
        endpoint = f"/mgmt/tm/ltm/node/{paths['node']}"
    ...
    def _handle_describe_node(self, param):
        ...
        if paths is None:
            return action_result.get_status()
        ret_val, response = self._make_rest_call(f"/mgmt/tm/ltm/node/{paths['node']}", action_result)
    ...
    def _handle_create_pool(self, param):
        ...
        if pool_description:
            ...
            # \ --> \\\\ which gets represented as \\\\\\\\ in the Python string
            pool_description = (
                self._handle_py_ver_compat_for_input_str(self._python_version, param.get("pool_description"))
                .replace("\\", "\\\\\\\\")
                .replace('"', '\\\\\\"')
            )
            json_str = f'{{"name": "{pool_name}", "partition": "{partition_name}", "description": "{pool_description}"}}'
        else:
            json_str = f'{{"name": "{pool_name}", "partition": "{partition_name}"}}'
        # make rest call
    ...
    def _handle_delete_pool(self, param):
        ...
        if paths is None:
            return action_result.get_status()
        ret_val, response = self._make_rest_call(f"/mgmt/tm/ltm/pool/{paths['pool']}", action_result, method="delete")
    ...
    def _handle_list_members(self, param):
        ...
        paths = self._quote_path_components(action_result, partition=partition_name, pool=pool_name)
        if paths is None:
            return action_result.get_status()
        response = self._paginator(f"/mgmt/tm/ltm/pool/~{paths['partition']}~{paths['pool']}/members", action_result, limit=max_results)
    ...
    def _handle_get_node_stats(self, param):
        ...
        if paths is None:
            return action_result.get_status()
        ret_val, response = self._make_rest_call(f"/mgmt/tm/ltm/node/{paths['node']}/stats", action_result)
        ...
        try:
            ...
            message = f"Unexpected API response. Error: {e}"
